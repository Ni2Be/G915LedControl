...
LEDController::LEDController()
    ...
    isPulsing(false), isRainbowWave(false), pulseDuration(2000.0f),
...
void LEDController::StartPulseEffect(int red, int green, int blue, float duration) {
...
void LEDController::PulseKeys() {
    ...
    while (running && isPulsing) {
        ...
        float intensity = (std::cos(t * 2 * 3.14159f) + 1) / 2;  // Oscillate between 0 and 1

        unsigned char bitmap[LOGI_LED_BITMAP_SIZE] = { 0 };
        int r = static_cast<int>(currentRed * intensity * 2.55f);
        int g = static_cast<int>(currentGreen * intensity * 2.55f);
        int b = static_cast<int>(currentBlue * intensity * 2.55f);
